const translations = {
    ...
    "The story of the hovercraft, which we named Cheburashka, began in September 2025. As part of the Introduction to Digital Manufacturing course, we were tasked with building a device with a non-standard design. That’s how I came up with the idea to build Cheburashka.": "История аэроглиссера, который мы назвали <span class=\"highlight\">Cheburashka</span>, началась в сентябре 2025 года. В рамках курса <span class=\"highlight\">«Введение в цифровое производство»</span> нам поручили создать устройство нестандартной конструкции. Так появилась идея Cheburashka.",
    "I didn’t want to copy anything from the internet, so I built it exactly as I had imagined it. By that point, I had already been working with drones and knew how to design models in Blender, so the development process got off to a quick start. Yes, Blender is the most user-friendly program for me, and it’s fairly easy to turn it into a decent CAD system using add-ons. But most importantly, Blender is very forgiving of mistakes, and that will save you more than once.": "<span class=\"highlight\">Я не хотел копировать что-либо из интернета, поэтому создал проект именно таким, каким его представлял.</span> К тому моменту я уже работал с дронами и умел проектировать модели в <span class=\"highlight\">Blender</span>, поэтому разработка началась быстро. <span class=\"highlight\">Blender — самая удобная для меня программа, а с помощью дополнений её легко превратить в достойную CAD-систему.</span> Главное — Blender прощает ошибки, и это не раз вас выручит.",
